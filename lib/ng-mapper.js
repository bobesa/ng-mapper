...
var regexpModule = new RegExp("angular.module\\(([A-Za-z0-9\"\\[\\]\', ]+)\\)","gm"),
    ...
    regexpModuleComponentSplit = new RegExp("\\.(service|directive|controller|factory)","g"),
...
if(!mainModule || mainModule == "") {
    console.warn("Main module is not set. You cannot map dependencies without setting main module (use --main <module name>)");
    return;
}
...
var allModules = {},
    ...
    ignoredComponents = ["$q","$window","$location","$scope","$rootScope","$http","$timeout","$routeParams","$animate","$compile","$controller","$filter","$sce"];
...
//1) Read source files
var sourceFiles = readFolder("./","js").filter(function(item){
...
var markDependencies = function(moduleName){
    ...
    for(var componentName in module.components) if(module.components.hasOwnProperty(componentName)) {
        ...
        //If component is directive -> Check template contents for directives (if template is present)
        if(component.type == COMPONENT_TYPE_DIRECTIVE) {
            //Check for templateUrl
            var templateUrl = component.source.match(regexpTemplateUrl);
            if(templateUrl && templateUrl.length > 3) {
                markDependenciesFromHtml(fs.readFileSync(templateUrl[3]).toString());
                continue;
            }

            //Check for template
            var template = component.source.match(regexpTemplate);
            if(template && template.length > 3) {
                markDependenciesFromHtml(captureContent(template[0]));
                continue;
            }
...
for(componentName in allComponents) if(allComponents.hasOwnProperty(componentName)) {
    ...
    if(!component.isUsed() && component.type == COMPONENT_TYPE_DIRECTIVE) postWarning(WARNING_IGNORED_COMPONENT, component.module, " > '" + component.getName() + "' is not used");
}
