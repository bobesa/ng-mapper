...
for(var argId = 2; argId < argLen; argId++) {
    ...
    switch(paramName) {
        ...
        case "--export-hierarchy":
            exportHierarchyFilename = getParamValue(++argId);
            break;
        default:
...
//5) Export hierarchy (if wanted)
if(exportHierarchyFilename != "") {
    var obj = [];
    for(moduleName in allModules) if(allModules.hasOwnProperty(moduleName)) {
        var module = allModules[moduleName];
        obj.push({
            name: module.name,
            size: module.dependencies.length,
            dependencies: module.dependencies.filter(function(dependency){
                return allModules.hasOwnProperty(dependency);
            })
        });
    }
    fs.writeFileSync(exportHierarchyFilename + ".json",JSON.stringify(obj));
}